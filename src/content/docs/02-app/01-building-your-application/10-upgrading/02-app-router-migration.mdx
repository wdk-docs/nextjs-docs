...
title: 应用路由器增量采用指南
nav_title: 应用程序路由器迁移
description: 了解如何将现有的Next.js应用程序从页面路由器升级到应用程序路由器。
---

本指南将帮助您:

- [将 Next.js 应用程序从版本 12 更新到版本 13](#nextjs-version)
- [升级同时在`pages`和`app`目录中工作的功能](#upgrading-new-features)
- [将现有应用程序从`页面`增量迁移到`应用程序`](#migrating-from-pages-to-app)

## 正在升级

### Node.js 版本

Node.js 的最低版本现在是**v18.17**。请参阅[Node.js 文档](https://nodejs.org/docs/latest-v18.x/api/)了解更多信息。

### Next.js 版本

要更新到 Next.js 版本 13，请使用首选包管理器运行以下命令：
...
> **很高兴知道**: You may need to restart the ESLint server in VS Code for the ESLint changes to take effect. Open the Command Palette (`cmd+shift+p` on Mac; `ctrl+shift+p` on Windows) and search for `ESLint: Restart ESLint Server`.
...
> **很高兴知道**: This is the easiest migration path because it has the most comparable behavior to the `pages` directory.
...
> **很高兴知道**: If you previously used API routes to call an external API from the client, you can now use [Server Components](/docs/app/building-your-application/rendering/server-components) instead to securely fetch data. Learn more about [data fetching](/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating).
