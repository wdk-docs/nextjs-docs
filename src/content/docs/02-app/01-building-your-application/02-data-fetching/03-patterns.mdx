...
> **很高兴知道**: This also applies to layouts, since it's not possible to pass data between a parent layout and its children.
...
您可以将`cache`功能、`preload`模式和`server-only`包相结合，创建一个可在整个应用程序中使用的数据提取实用程序。
...
export async function getUserData() {
  ...
  experimental_taintUniqueValue("Do not pass the user's phone number to the client", data, data.phoneNumber);
  return data;
}
```
